...
```
There are no test projects yet. When adding one, use xUnit per
`~/.claude/rules/ecc/csharp/testing.md` and wire it into `Padel.sln`; run a single test with
...
- **Padel.Api** — `Program.cs` composition root: Serilog console logging, Swagger (dev only),
  CORS restricted to `Cors:AllowedOrigins` from config, JWT bearer auth package is referenced but
  not yet wired up in `Program.cs`. Migrations run automatically at startup
