...
// AdminLayout is the one place `.admin-scope` gets applied — every token-suppressible bit of
// Bounce (radius, tap-spring intensity, shadow glow) is dialed down here for every descendant
// page automatically. Admin JSX should also never import from components/illustrations/ —
// that half of "calm variant" isn't expressible as a token, so it's enforced by convention.
    ...
    <BounceScaleContext.Provider value={0.4}>
      <div className="admin-scope flex min-h-screen bg-background text-foreground">
        <aside className="hidden w-60 shrink-0 border-e border-border bg-sidebar p-4 md:flex md:flex-col">
          <div className="mb-6 px-2 font-display text-lg font-bold text-primary">Padel Aura</div>
        ...
        {menuOpen && (
          <div className="fixed inset-0 z-50 md:hidden">
            ...
            <nav className="absolute inset-y-0 start-0 flex w-64 max-w-[80%] flex-col gap-1 bg-sidebar p-4 shadow-candy-navy">
              <div className="mb-4 flex items-center justify-between px-2">
                <span className="font-display text-lg font-bold text-primary">Padel Aura</span>
                <Button
                  variant="ghost"
                  size="icon-sm"
